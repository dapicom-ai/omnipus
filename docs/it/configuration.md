...
File di configurazione: `~/.omnipus/config.json`

### Variabili d'Ambiente

Puoi sovrascrivere i percorsi predefiniti usando variabili d'ambiente. Questo è utile per installazioni portatili, distribuzioni containerizzate, o per eseguire omnipus come servizio di sistema. Queste variabili sono indipendenti e controllano percorsi diversi.
...
Omnipus salva i dati nel workspace configurato (predefinito: `~/.omnipus/workspace`):
...
Omnipus esegue in un ambiente sandboxed per impostazione predefinita. L'agent può accedere solo ai file ed eseguire comandi all'interno del workspace configurato.
...
| ----------------------- | ----------------------- | ---------------------------------------------------- |
| `workspace`             | `~/.omnipus/workspace` | Directory di lavoro dell'agent                       |
...
Il controllo di sicurezza exec ispeziona solo la riga di comando avviata direttamente da Omnipus. Non ispeziona ricorsivamente i processi figlio generati da strumenti di sviluppo consentiti come `make`, `go run`, `cargo`, `npm run` o script di build personalizzati.
...
* Esegui Omnipus in un container o VM se hai bisogno di un isolamento più forte di quello fornito dal controllo integrato.
...
Omnipus può eseguire task periodici automaticamente. Crea un file `HEARTBEAT.md` nel tuo workspace:
