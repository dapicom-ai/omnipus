...
| 10 | `cmd/omnipus/main.go` still shows Omnipus banner | **note** | `cmd/omnipus/main.go:57-66` | The ASCII art banner still says "OMNIPUS" in box characters despite the codebase rename. The command name is correctly `omnipus`. | Brand | Replace the Omnipus ASCII banner with Omnipus branding. Low priority but visible to every user on every launch. |
...
- Deprecating SessionManager requires migration path for existing Omnipus data
