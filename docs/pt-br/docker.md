...
Você também pode executar o Omnipus usando Docker Compose sem instalar nada localmente.
...
A imagem `launcher` inclui os três binários (`omnipus`, `omnipus-launcher`, `omnipus-launcher-tui`) e inicia o console web por padrão, que fornece uma interface baseada em navegador para configuração e chat.
...
> [!TIP]
> Configure sua chave de API em `~/.omnipus/config.json`. Obtenha chaves de API: [Volcengine (CodingPlan)](https://www.volcengine.com/activity/codingplan?utm_campaign=Omnipus&utm_content=Omnipus&utm_medium=devrel&utm_source=OWO&utm_term=Omnipus) (LLM) · [OpenRouter](https://openrouter.ai/keys) (LLM) · [Zhipu](https://open.bigmodel.cn/usercenter/proj-mgmt/apikeys) (LLM). A busca na web é opcional — obtenha gratuitamente uma [API Tavily](https://tavily.com) (1000 consultas gratuitas/mês) ou [API Brave Search](https://brave.com/search/api) (2000 consultas gratuitas/mês).
...
> `request_timeout` é opcional e usa segundos. Se omitido ou definido como `<= 0`, o Omnipus usa o timeout padrão (120s).
