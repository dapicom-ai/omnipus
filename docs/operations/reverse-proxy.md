...
The Omnipus gateway listens on two TCP ports. The main port (default **5000**) serves the SPA and the authenticated REST/WebSocket/SSE API. The preview port (default **5001**) serves agent-generated HTML, CSS, and JavaScript via time-limited bearer tokens — these are the same assets produced by `tools.serve_workspace` and surfaced in the chat as clickable previews.
...
When `preview_listener_enabled` is `false`, the iframe-preview feature is disabled entirely. The gateway does not start the second listener, and the `/serve/` and `/dev/` path prefixes are not registered on the main mux either — any browser request to `<main-host>:<port>/serve/...` or `<main-host>:<port>/dev/...` will receive a 404 from the SPA catch-all handler. Tool calls to `serve_workspace` and `run_in_workspace` will still register tokens in the in-memory registry, but the URLs they return will point at the (unregistered) main-port paths and will not resolve. This is a full rollback of the iframe-preview feature, not a partial-degradation mode. If you need to restore functionality, re-enable the preview listener and restart the gateway.
