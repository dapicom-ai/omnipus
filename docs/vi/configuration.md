...
File cấu hình: `~/.omnipus/config.json`

### Biến Môi Trường

Bạn có thể ghi đè các đường dẫn mặc định bằng biến môi trường. Điều này hữu ích cho cài đặt portable, triển khai container, hoặc chạy omnipus như dịch vụ hệ thống. Các biến này độc lập và kiểm soát các đường dẫn khác nhau.
...
Omnipus lưu trữ dữ liệu trong workspace đã cấu hình (mặc định: `~/.omnipus/workspace`):
...
Omnipus chạy trong môi trường sandbox mặc định. Agent chỉ có thể truy cập file và thực thi lệnh trong workspace đã cấu hình.
...
| ----------------------- | ----------------------- | ----------------------------------------- |
| `workspace`             | `~/.omnipus/workspace` | Thư mục làm việc của agent               |
...
Guard bảo mật exec chỉ kiểm tra dòng lệnh mà Omnipus khởi chạy trực tiếp. Nó không kiểm tra đệ quy các tiến trình con được tạo bởi công cụ phát triển được phép như `make`, `go run`, `cargo`, `npm run`, hoặc script build tùy chỉnh.
...
* Chạy Omnipus trong container hoặc VM nếu bạn cần cách ly mạnh hơn guard tích hợp.
...
Omnipus có thể thực hiện tác vụ định kỳ tự động. Tạo file `HEARTBEAT.md` trong workspace:
...
| `volcengine` | LLM (Volcengine trực tiếp)              | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=Omnipus&utm_content=Omnipus&utm_medium=devrel&utm_source=OWO&utm_term=Omnipus) |
...
> **Tính năng mới:** Omnipus hiện sử dụng cách tiếp cận **lấy mô hình làm trung tâm**. Chỉ cần chỉ định định dạng `vendor/model` (ví dụ: `zhipu/glm-4.7`) để thêm provider mới — **không cần thay đổi code!**
...
| **VolcEngine (Doubao)** | `volcengine/`   | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI    | [Lấy](https://www.volcengine.com/activity/codingplan?utm_campaign=Omnipus&utm_content=Omnipus&utm_medium=devrel&utm_source=OWO&utm_term=Omnipus) |
| **Antigravity**         | `antigravity/`  | Google Cloud                                        | Custom    | Chỉ OAuth                                                        |

#### Cân Bằng Tải

Cấu hình nhiều endpoint cho cùng tên mô hình — Omnipus sẽ tự động round-robin:
...
Omnipus định tuyến provider theo họ giao thức:
...
Omnipus hỗ trợ tác vụ theo lịch qua công cụ `cron`.
...
Tác vụ đã lên lịch được lưu trữ bền vững sau khi khởi động lại tại `~/.omnipus/workspace/cron/`.
