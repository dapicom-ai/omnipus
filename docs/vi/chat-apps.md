...
Trò chuyện với omnipus của bạn qua Telegram, Discord, WhatsApp, Matrix, QQ, DingTalk, LINE, WeCom, Feishu, Slack, IRC, OneBot hoặc MaixCam
...
| **Pico**             | ⭐ Dễ              | Kênh giao thức bản địa Omnipus                       |                                                                                                                  |
...
Omnipus hiện lưu trữ định nghĩa lệnh trong một registry chung. Khi khởi động, Telegram sẽ tự động đăng ký các lệnh bot được hỗ trợ (ví dụ `/start`, `/help`, `/show`, `/list`) để menu lệnh và hành vi runtime luôn đồng bộ.
...
Nếu đăng ký lệnh thất bại (lỗi tạm thời mạng/API), kênh vẫn khởi động và Omnipus thử lại đăng ký trong nền.
...
Omnipus có thể kết nối WhatsApp theo hai cách:
...
Nếu `session_store_path` trống, phiên được lưu tại `<workspace>/whatsapp/`. Chạy `omnipus gateway`; lần chạy đầu tiên, quét mã QR hiển thị trong terminal bằng WhatsApp → Thiết bị liên kết.
...
Omnipus hỗ trợ kết nối với tài khoản WeChat cá nhân của bạn thông qua API chính thức Tencent iLink.
...
4. Chạy `omnipus gateway` và mở QQ để trò chuyện với bot của bạn
...
* Cấu hình như trên và chạy `omnipus gateway`
...
Omnipus hỗ trợ ba loại tích hợp WeCom:
...
Omnipus kết nối với Feishu qua chế độ WebSocket/SDK — không cần URL webhook công khai hay máy chủ callback.
...
OneBot là giao thức mở cho bot QQ. Omnipus kết nối với bất kỳ triển khai tương thích OneBot v11 nào (ví dụ: [Lagrange](https://github.com/LagrangeDev/Lagrange.Core), [NapCat](https://github.com/NapNeko/NapCatQQ)) qua WebSocket.
