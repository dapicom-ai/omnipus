...
設定ファイルパス: `~/.omnipus/config.json`

### 環境変数

環境変数を使用してデフォルトパスを上書きできます。ポータブルインストール、コンテナ化デプロイ、または omnipus をシステムサービスとして実行する場合に便利です。これらの変数は独立しており、異なるパスを制御します。

| 変数              | 説明                                                                                                                             | デフォルトパス            |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `OMNIPUS_CONFIG` | 設定ファイルのパスを上書きします。omnipus がどの `config.json` を読み込むかを直接指定し、他のすべての場所を無視します。 | `~/.omnipus/config.json` |
| `OMNIPUS_HOME`   | omnipus データのルートディレクトリを上書きします。`workspace` やその他のデータディレクトリのデフォルト場所を変更します。          | `~/.omnipus`             |

**例：**

```bash
# 特定の設定ファイルで omnipus を実行
# ワークスペースパスはその設定ファイル内から読み込まれます
OMNIPUS_CONFIG=/etc/omnipus/production.json omnipus gateway

# /opt/omnipus にすべてのデータを保存して omnipus を実行
# 設定はデフォルトの ~/.omnipus/config.json から読み込まれます
# ワークスペースは /opt/omnipus/workspace に作成されます
...
Omnipus は設定されたワークスペース（デフォルト: `~/.omnipus/workspace`）にデータを保存します：
...
1. `~/.omnipus/workspace/skills`（ワークスペース）
2. `~/.omnipus/skills`（グローバル）
...
Omnipus はデフォルトでサンドボックス環境で実行されます。Agent は設定されたワークスペース内のファイルアクセスとコマンド実行のみが可能です。
...
| オプション              | デフォルト値            | 説明                                  |
| ----------------------- | ----------------------- | ------------------------------------- |
| `workspace`             | `~/.omnipus/workspace` | Agent の作業ディレクトリ              |
...
exec セキュリティガードは Omnipus が直接起動するコマンドラインのみを検査します。`make`、`go run`、`cargo`、`npm run`、またはカスタムビルドスクリプトなどの開発ツールが生成する子プロセスは再帰的に検査しません。
...
* ビルトインガードより強力な分離が必要な場合は、コンテナまたは VM 内で Omnipus を実行してください。
...
Omnipus は定期タスクを自動実行できます。ワークスペースに `HEARTBEAT.md` ファイルを作成してください：
...
- `OMNIPUS_HEARTBEAT_ENABLED=false` で無効化
- `OMNIPUS_HEARTBEAT_INTERVAL=60` で間隔を変更
...
| `volcengine` | LLM（Volcengine 直接）                  | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=Omnipus&utm_content=Omnipus&utm_medium=devrel&utm_source=OWO&utm_term=Omnipus) |
...
> **新機能：** Omnipus は**モデル中心**の設定アプローチを採用しました。`vendor/model` 形式（例：`zhipu/glm-4.7`）を指定するだけで新しい Provider を追加できます — **コード変更不要！**
...
| **VolcEngine (Doubao)** | `volcengine/`          | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI     | [取得](https://www.volcengine.com/activity/codingplan?utm_campaign=Omnipus&utm_content=Omnipus&utm_medium=devrel&utm_source=OWO&utm_term=Omnipus) |
| **Antigravity**         | `antigravity/`         | Google Cloud                                        | Custom     | OAuth のみ                                                       |

#### ロードバランシング

同じモデル名に複数のエンドポイントを設定すると、Omnipus が自動的にラウンドロビンします：
...
Omnipus はプロトコルファミリーで Provider をルーティングします：
...
Omnipus は `cron` ツールを通じて cron スタイルのスケジュールタスクをサポートします。
...
スケジュールタスクは再起動後も `~/.omnipus/workspace/cron/` に保存されます。
