<div align="center">
  ...
  <h1>Omnipus: Go で書かれた超効率 AI アシスタント</h1>

  <h3>$10 ハードウェア · 10MB RAM · ms 起動 · Let's Go, Omnipus!</h3>
  <p>
...
> **Omnipus** は [Sipeed](https://sipeed.com) が立ち上げた独立したオープンソースプロジェクトです。完全に **Go 言語**で一から書かれており、OpenClaw、NanoBot、その他のプロジェクトのフォークではありません。

**Omnipus** は [NanoBot](https://github.com/HKUDS/nanobot) にインスパイアされた超軽量パーソナル AI アシスタントです。**Go** でゼロからリビルドされ、「セルフブートストラッピング」プロセスで構築されました — AI Agent 自身がアーキテクチャの移行とコード最適化を推進しました。
...
> **セキュリティに関する注意**
>
> * **暗号通貨なし:** Omnipus には公式トークン/コインは**一切ありません**。`pump.fun` やその他の取引プラットフォームでの主張はすべて**詐欺**です。
> * **公式ドメイン:** **唯一**の公式サイトは **[omnipus.io](https://omnipus.io)**、企業サイトは **[sipeed.com](https://sipeed.com)** です。
> * **注意:** 多くの `.ai/.org/.com/.net/...` ドメインは第三者によって登録されています。信頼しないでください。
> * **注記:** Omnipus は初期開発段階にあり、未解決のネットワークセキュリティ問題がある可能性があります。v1.0 リリース前に本番環境へのデプロイは避けてください。
> * **注記:** Omnipus は最近多くの PR をマージしており、最新バージョンではメモリフットプリントが大きくなる場合があります（10〜20MB）。機能セットが安定次第、リソース最適化を優先する予定です。

## 📢 ニュース

2026-03-25 🚀 **v0.2.4 リリース！** Agent アーキテクチャ全面刷新（SubTurn、Hooks、Steering、EventBus）、WeChat/WeCom 統合、セキュリティ強化（.security.yml、機密データフィルタリング）、新プロバイダー（AWS Bedrock、Azure、Xiaomi MiMo）、35 件のバグ修正。Omnipus **26K ⭐** 達成！

2026-03-17 🚀 **v0.2.3 リリース！** システムトレイ UI（Windows & Linux）、サブエージェントステータス追跡（`spawn_status`）、実験的 Gateway ホットリロード、cron セキュリティゲート、セキュリティ修正 2 件。Omnipus **25K ⭐** 達成！
...
2026-02-26 🎉 Omnipus がわずか 17 日で **20K スター** 達成！Channel 自動オーケストレーションとケイパビリティインターフェースが実装されました。

2026-02-16 🎉 Omnipus が 1 週間で 12K スター達成！コミュニティメンテナーの役割と[ロードマップ](ROADMAP.md)が正式に公開されました。

2026-02-13 🎉 Omnipus が 4 日間で 5000 スター達成！プロジェクトロードマップと開発者グループの準備が進行中。

2026-02-09 🎉 **Omnipus リリース！** $10 ハードウェアで 10MB 未満の RAM で動く AI Agent を 1 日で構築。Let's Go, Omnipus!
...
Omnipus はほぼすべての Linux デバイスにデプロイできます！
...
### omnipus.io からダウンロード（推奨）

**[omnipus.io](https://omnipus.io)** にアクセス — 公式サイトがプラットフォームを自動検出し、ワンクリックでダウンロードできます。アーキテクチャを手動で選ぶ必要はありません。

### プリコンパイル済みバイナリをダウンロード

または、[GitHub Releases](https://github.com/sipeed/omnipus/releases) ページからプラットフォームに合ったバイナリをダウンロードしてください。
...
[omnipus.io](https://omnipus.io) からダウンロード後、`omnipus-launcher`（Windows では `omnipus-launcher.exe`）をダブルクリックしてください。ブラウザが自動的に `http://localhost:18800` を開きます。
...
WebUI の詳細なドキュメントは [docs.omnipus.io](https://docs.omnipus.io) を参照してください。
...
`omnipus-launcher` はインターネットからダウンロードされ、Mac App Store を通じて公証されていないため、macOS が初回起動時にブロックする場合があります。

**ステップ 1：** `omnipus-launcher` をダブルクリックすると、セキュリティ警告が表示されます：

<p align="center">
<img src="assets/macos-gatekeeper-warning.jpg" alt="macOS Gatekeeper 警告" width="400">
</p>

> *"omnipus-launcher" は開けません — "omnipus-launcher" がMacに害を与えたりプライバシーを侵害するマルウェアを含まないことをAppleは確認できません。*
...
TUI の詳細なドキュメントは [docs.omnipus.io](https://docs.omnipus.io) を参照してください。

### 📱 Android

10 年前のスマホに第二の人生を！Omnipus でスマート AI アシスタントに変身させましょう。
...
pkg install proot
termux-chroot ./omnipus onboard   # chroot で標準的な Linux ファイルシステムレイアウトを提供
```
...
`omnipus` コアバイナリのみが利用可能な最小環境（Launcher UI なし）では、コマンドラインと JSON 設定ファイルですべてを設定できます。
...
`~/.omnipus/config.json` とワークスペースディレクトリが作成されます。

**2. 設定** (`~/.omnipus/config.json`)
...
Omnipus は `model_list` 設定を通じて 30 以上の LLM Provider をサポートしています。`protocol/model` 形式を使用してください：
...
17 以上のメッセージングプラットフォームで Omnipus と会話できます：
...
Omnipus は最新情報を提供するために Web を検索できます。`tools.web` で設定してください：
...
Omnipus にはファイル操作、コード実行、スケジューリングなどの組み込みツールが含まれています。詳細は [ツール設定](docs/ja/tools_configuration.md) を参照してください。
...
Omnipus は [MCP](https://modelcontextprotocol.io/) をネイティブサポートしています — 任意の MCP サーバーに接続して、外部ツールやデータソースで Agent の機能を拡張できます。
...
CLI または統合チャットアプリからメッセージを 1 つ送るだけで、Omnipus をエージェントソーシャルネットワークに接続できます。
...
| コマンド                  | 説明                           |
| ------------------------- | ------------------------------ |
| `omnipus onboard`        | 設定＆ワークスペースの初期化     |
| `omnipus auth weixin` | WeChat アカウントを QR で接続 |
| `omnipus agent -m "..."` | Agent とチャット                |
| `omnipus agent`          | インタラクティブチャットモード   |
| `omnipus gateway`        | Gateway を起動                  |
| `omnipus status`         | ステータスを表示                |
| `omnipus version`        | バージョン情報を表示            |
| `omnipus model`          | デフォルトモデルの表示・切替    |
| `omnipus cron list`      | スケジュールジョブ一覧          |
| `omnipus cron add ...`   | スケジュールジョブを追加         |
| `omnipus cron disable`   | スケジュールジョブを無効化       |
| `omnipus cron remove`    | スケジュールジョブを削除         |
| `omnipus skills list`    | インストール済み Skill 一覧      |
| `omnipus skills install` | Skill をインストール             |
| `omnipus migrate`        | 旧バージョンからデータを移行     |
| `omnipus auth login`     | Provider への認証               |

### ⏰ スケジュールタスク / リマインダー

Omnipus は `cron` ツールによるスケジュールリマインダーと定期タスクをサポートしています：
...
[コミュニティロードマップ](https://github.com/sipeed/omnipus/issues/988)と[CONTRIBUTING.md](CONTRIBUTING.md)をご覧ください。
