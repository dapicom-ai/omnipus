...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("omnipus")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@omnipus hello", want: true},
		{text: "hi @omnipus:matrix.org", want: true},
		{
			text: "\u6b22\u8fce\u4e00\u4e0bomnipus\u5c0f\u9f99\u867e",
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@omnipus:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@omnipus:matrix.org hello", want: "hello"},
		{in: "@omnipus, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@omnipus:matrix.org")},
				},
			},
			want: true,
		},
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/@omnipus:matrix.org">Omnipus</a> hello`,
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/%40omnipus%3Amatrix.org">Omnipus</a> hello`,
			},
			want: true,
		},
	}
...
func TestDownloadMedia_WritesResponseToTempFile(t *testing.T) {
	...
	client, err := mautrix.NewClient(server.URL, id.UserID("@omnipus:matrix.test"), "")
	if err != nil {
