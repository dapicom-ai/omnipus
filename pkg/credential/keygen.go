...
// DefaultSSHKeyPath returns the canonical path for the omnipus-specific SSH key.
// The path is always ~/.ssh/omnipus_ed25519.key (os.UserHomeDir is cross-platform).
func DefaultSSHKeyPath() (string, error) {
	home, err := os.UserHomeDir()
	if err != nil {
		return "", fmt.Errorf("credential: cannot determine home directory: %w", err)
	}
	return filepath.Join(home, ".ssh", "omnipus_ed25519.key"), nil
}
