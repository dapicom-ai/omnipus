...
func repairHistory(
	...
	if len(orphanUses) > 0 {
		out = injectSyntheticToolResults(ctx, out, orphanUses, transcriptIndex, toolRegistry, agentID, &stats)
	}
...
func injectSyntheticToolResults(
	...
	for i < len(messages) {
		...
		for _, id := range ids {
			...
			content, handled := tryReinvokeIdempotent(ctx, id, entry, toolRegistry, agentID, stats)
			if !handled {
