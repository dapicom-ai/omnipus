...
func TestRunRecap_HappyPath_PersistsLastSessionAndRetro(t *testing.T) {
	...
	al := NewAgentLoop(cfg, msgBus, script)
...
// allow-list, boot must fail with a clear config error. We cannot call
// NewAgentLoop directly in the same process because the implementation calls
// os.Exit(1) — so we re-execute this test binary with a sentinel env var that
// triggers the path, and assert the child exited non-zero.
...
func TestBootstrapRecapPass_OneIterationPerSession(t *testing.T) {
	...
	// Claim map must contain the sessionID exactly once regardless of N agents.
	if _, ok := al.claimedCloseSessions.Load(sessionID); !ok {
		t.Error("claim map missing sessionID after bootstrap enqueue")
	}
	if count := syncMapLenLocal(&al.claimedCloseSessions); count != 1 {
		t.Errorf("bootstrap pass claimed %d sessions; expected exactly 1", count)
	}
