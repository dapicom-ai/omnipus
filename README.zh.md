...
<h1>Omnipus: 基于Go语言的超高效 AI 助手</h1>
...
> **Omnipus** 是由 [矽速科技 (Sipeed)](https://sipeed.com) 发起的独立开源项目，完全使用 **Go 语言**从零编写——不是 OpenClaw、NanoBot 或其他项目的分支。

🦐 **Omnipus** 是一个受 [NanoBot](https://github.com/HKUDS/nanobot) 启发的超轻量级个人 AI 助手。它采用 **Go 语言** 从零重构，经历了一个"自举"过程——即由 AI Agent 自身驱动了整个架构迁移和代码优化。
...
> **🚨 安全声明**
>
> - **无加密货币 (NO CRYPTO):** Omnipus **没有** 发行任何官方代币、Token 或虚拟货币。所有在 `pump.fun` 或其他交易平台上的相关声称均为 **诈骗**。
> - **官方域名:** 唯一的官方网站是 **[omnipus.io](https://omnipus.io)**，公司官网是 **[sipeed.com](https://sipeed.com)**。
> - **警惕:** 许多 `.ai/.org/.com/.net/...` 后缀的域名被第三方抢注，请勿轻信。
> - **注意:** Omnipus 正在初期的快速功能开发阶段，可能有尚未修复的网络安全问题，在 1.0 正式版发布前，请不要将其部署到生产环境中。
> - **注意:** Omnipus 最近合并了大量 PR，近期版本可能内存占用较大 (10~20MB)，我们将在功能较为收敛后进行资源占用优化。

## 📢 新闻

2026-03-25 🚀 **v0.2.4 发布！** Agent 架构全面重构（SubTurn、Hook、Steering、EventBus）、微信/企业微信深度集成、安全体系升级（.security.yml、敏感数据过滤）、新增 Provider（AWS Bedrock、Azure、小米 MiMo），以及 35 项 Bug 修复。Omnipus 已达 **26K ⭐**！

2026-03-17 🚀 **v0.2.3 发布！** 系统托盘 UI（Windows & Linux）、子 Agent 状态查询 (`spawn_status`)、实验性 Gateway 热重载、Cron 安全门控，以及 2 项安全修复。Omnipus 已达 **25K ⭐**！
...
2026-02-26 🎉 Omnipus 仅 17 天突破 **20K Stars**！频道自动编排和能力接口上线。

2026-02-16 🎉 Omnipus 一周内突破 12K Stars！社区维护者角色和 [路线图](ROADMAP.md) 正式发布。

2026-02-13 🎉 Omnipus 4 天内突破 5000 Stars！项目路线图和开发者群组筹建中。

2026-02-09 🎉 **Omnipus 正式发布！** 仅用 1 天构建，将 AI Agent 带入 $10 硬件与 <10MB 内存的世界。🦐 皮皮虾，我们走！
...
访问 **[omnipus.io](https://omnipus.io)** — 官网自动检测你的平台，提供一键下载，无需手动选择架构。

### 下载预编译二进制文件

也可以从 [GitHub Releases](https://github.com/sipeed/omnipus/releases) 页面手动下载对应平台的二进制文件。
...
从 [omnipus.io](https://omnipus.io) 下载后，双击 `omnipus-launcher`（Windows 上为 `omnipus-launcher.exe`），浏览器将自动打开 `http://localhost:18800`。
...
详细 WebUI 文档请参阅 [docs.omnipus.io](https://docs.omnipus.io)。
...
macOS 可能会在首次启动时拦截 `omnipus-launcher`，因为它从互联网下载，未经 Mac App Store 公证。

**第一步：** 双击 `omnipus-launcher`，会出现安全警告：

<p align="center">
<img src="assets/macos-gatekeeper-warning.jpg" alt="macOS Gatekeeper 警告" width="400">
</p>

> *"omnipus-launcher" 无法打开 — Apple 无法验证 "omnipus-launcher" 不含可能损害 Mac 或危及隐私的恶意软件。*
...
完成这一次操作后，后续启动 `omnipus-launcher` 将不再弹出警告。
...
详细 TUI 文档请参阅 [docs.omnipus.io](https://docs.omnipus.io)。
...
pkg install proot
termux-chroot ./omnipus onboard   # chroot 提供标准 Linux 文件系统布局
```
...
对于只有 `omnipus` 核心二进制文件的极简环境（无 Launcher UI），可通过命令行和 JSON 配置文件完成所有配置。
...
此命令会创建 `~/.omnipus/config.json` 和工作区目录。

**2. 配置** (`~/.omnipus/config.json`)
...
Omnipus 通过 `model_list` 配置支持 30+ LLM Provider，使用 `协议/模型` 格式：
...
Omnipus 可以搜索网络以提供最新信息。在 `tools.web` 中配置：
...
Omnipus 内置文件操作、代码执行、定时任务等工具。详情请参阅 [工具配置](docs/zh/tools_configuration.md)。
...
Omnipus 原生支持 [MCP](https://modelcontextprotocol.io/) — 连接任意 MCP 服务器，通过外部工具和数据源扩展 Agent 能力。
...
通过 CLI 或任何已集成的聊天应用发送一条消息，即可将 Omnipus 连接到 Agent 社交网络。
...
| 命令                       | 说明                   |
| ------------------------- | ---------------------- |
| `omnipus onboard`        | 初始化配置与工作区       |
| `omnipus auth weixin` | 扫码连接微信个人号 |
| `omnipus agent -m "..."` | 与 Agent 对话           |
| `omnipus agent`          | 交互式对话模式           |
| `omnipus gateway`        | 启动网关                |
| `omnipus status`         | 查看状态                |
| `omnipus version`        | 查看版本信息             |
| `omnipus model`          | 查看或切换默认模型       |
| `omnipus cron list`      | 列出所有定时任务         |
| `omnipus cron add ...`   | 添加定时任务             |
| `omnipus cron disable`   | 禁用定时任务             |
| `omnipus cron remove`    | 删除定时任务             |
| `omnipus skills list`    | 列出已安装 Skills        |
| `omnipus skills install` | 安装 Skill              |
| `omnipus migrate`        | 从旧版本迁移数据         |
| `omnipus auth login`     | 认证 Provider           |

### ⏰ 定时任务 / 提醒

Omnipus 通过 `cron` 工具支持定时提醒和重复任务：
...
查看完整的 [社区路线图](https://github.com/sipeed/omnipus/issues/988) 和 [CONTRIBUTING.md](CONTRIBUTING.md)。
