...
const (
	autoStartEntryName = "OmnipusLauncher"
	launchAgentLabel   = "io.omnipus.launcher"
)
...
func linuxAutoStartPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "autostart", "omnipus-web.desktop")
}
...
func setLinuxAutoStart(enabled bool, exePath string, args []string) error {
	...
	if enabled {
		...
		content := strings.Join([]string{
			...
			"Version=1.0",
			"Name=Omnipus Web",
			"Comment=Start Omnipus Web on login",
