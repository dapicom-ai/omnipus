...
func TestGatewayRestartReturnsErrorStatusWhenReplacementFailsToStart(t *testing.T) {
	...
	invalidBinaryPath := filepath.Join(t.TempDir(), "fake-omnipus")
...
func TestFindOmnipusBinary_EnvOverride(t *testing.T) {
	// Create a temporary file to act as the mock binary
	tmpDir := t.TempDir()
	mockBinary := filepath.Join(tmpDir, "omnipus-mock")
	...
	got := utils.FindOmnipusBinary()
	if got != mockBinary {
		t.Errorf("FindOmnipusBinary() = %q, want %q", got, mockBinary)
	}
}

func TestFindOmnipusBinary_EnvOverride_InvalidPath(t *testing.T) {
	// When PICOCLAW_BINARY points to a non-existent path, fall through to next strategy
	t.Setenv("PICOCLAW_BINARY", "/nonexistent/omnipus-binary")

	got := utils.FindOmnipusBinary()
	// Should not return the invalid path; falls back to "omnipus" or another found path
	if got == "/nonexistent/omnipus-binary" {
		t.Errorf("FindOmnipusBinary() returned invalid env path %q, expected fallback", got)
	}
}
